...
//	}
	...
	@PostMapping("customer/cart/insert/pseq={pseq}/quantity={quantity}")
	public String action(@PathVariable("pseq") int pseq, @PathVariable("quantity") int quantity, HttpServletRequest request, HttpSession session) {
		Customer customer = (Customer)session.getAttribute("cAuthInfo");
		
		if(customer != null) {
			cartDaoImpl.updateCartQuantity(customer.getEmail(), pseq, quantity);
	...
	@GetMapping("customer/cartAlarm")
	public String cartAlarmForm(HttpSession session) {
		Customer customer = (Customer)session.getAttribute("cAuthInfo");
		if(customer != null) {
			return "customer/cartAlarmForm";
		}else {
			return "redirect:/customer/login";
		}
